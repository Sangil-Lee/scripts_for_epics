...
def print_usage():
    ...
    print("      예1) python3 calc_waveform.py IN OUT ALL PWM:5:50")
    print("           -> 5번 깜빡이는 완벽한 사각파 (50% 듀티, 진폭 1.0)")
    print("      예2) python3 calc_waveform.py IN OUT ALL PWM:10:20 5.0")
    print("           -> 10주기, 20% 시간만 켜지는 짧은 펄스 (진폭 5.0)\n")
...
def main():
    ...
    # --- 2. 데이터 생성 (Generate) ---
        ...
        # PWM:cycle:duty (기본값: 1주기, 50% 듀티)
        cycles = float(op_parts[1]) if len(op_parts) > 1 else 1.0
        duty = float(op_parts[2]) if len(op_parts) > 2 else 50.0
        
        # 5번째 인자(operand)가 0이 아니면 진폭으로 사용, 아니면 1.0
        high_val = operand if operand != 0.0 else 1.0 
        
        # 0.0 ~ 1.0 사이를 cycles 만큼 반복하는 위상(phase) 배열 생성
        phase = np.linspace(0, cycles, size, endpoint=False) % 1.0
        
        # phase 값이 duty 비율(0~1)보다 작으면 high_val, 크면 0.0을 채움
